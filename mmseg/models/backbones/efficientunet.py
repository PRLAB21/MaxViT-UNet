...
from mmseg.models.utils.efficientnet_layers import *
# from .efficientnet import EfficientNet
from .efficientnet_mmcls import EfficientNet
...
class EfficientUNet(BaseModule):
    def __init__(self, model_name='efficientnet-b5', out_channels=1, concat_input=True, pretrained=True, debug=False):
        super(EfficientUNet, self).__init__()

        self.module_name = '[EfficientUNet]'
        self.debug = debug

        # self.encoder = EfficientNet.encoder(model_name, pretrained=pretrained)
        print('[before]', self.n_channels)
        self.encoder = EfficientNet(arch='b0', out_indices=(1, 2, 3, 4, 6))
        print('[after]', self.n_channels)
    ...
    def forward(self, x):
        ...
        if self.debug: print(self.module_name, '[blocks]', len(blocks))
        for key in blocks:
            if self.debug: print(self.module_name, f'[blocks][{key}]', blocks[key].shape)
        _, x = blocks.popitem()
        if self.debug: print(self.module_name, '[x][popitem]', x.shape)
        dec_outs.append(x)

        x = self.up_conv1(x)
        if self.debug: print(self.module_name, '[x][up_conv1]', x.shape)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        if self.debug: print(self.module_name, '[x][cat]', x.shape)
        x = self.double_conv1(x)
        if self.debug: print(self.module_name, '[x][double_conv1]', x.shape)
        dec_outs.append(x)

        x = self.up_conv2(x)
        if self.debug: print(self.module_name, '[x][up_conv2]', x.shape)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        if self.debug: print(self.module_name, '[x][cat]', x.shape)
        x = self.double_conv2(x)
        if self.debug: print(self.module_name, '[x][double_conv2]', x.shape)
        dec_outs.append(x)

        x = self.up_conv3(x)
        if self.debug: print(self.module_name, '[x][up_conv3]', x.shape)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        if self.debug: print(self.module_name, '[x][cat]', x.shape)
        x = self.double_conv3(x)
        if self.debug: print(self.module_name, '[x][double_conv3]', x.shape)
        dec_outs.append(x)

        x = self.up_conv4(x)
        if self.debug: print(self.module_name, '[x][up_conv4]', x.shape)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        if self.debug: print(self.module_name, '[x][cat]', x.shape)
        x = self.double_conv4(x)
        if self.debug: print(self.module_name, '[x][double_conv4]', x.shape)
        dec_outs.append(x)
        ...
        # x = self.final_conv(x)
        # if self.debug: print(self.module_name, '[x][final_conv]', x.shape)
        # dec_outs.append(x)
