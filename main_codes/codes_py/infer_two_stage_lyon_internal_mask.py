...
for i, f in tqdm(enumerate(fnames), total=len(fnames), position=0, leave=True):
    ...
    if TWO_STAGE:
        if predicted_label == 0 or predicted_label == 1:  # filter artifacts and normal patches
            ...
            #     result_analysis.append(result)
                # predicted_count = len(predicted_boxes)
            masks = [np.where(m == 1, 255, m) for m in output_beta[1]]
            for m in masks:
                
                
                # bin_mask= bin_mask.astype(np.uint8)  #convert to an unsigned byte
                   # bin_mask*=255
                ...
                # properties = [fname, area, e]
                    if (e >= 1) or (area < 50):
                     pred2ignore.append([f, area, e])
                    else:
                        morph_predictions.append([f, area, e])
                        temp.extend([f, area, e])

            predicted_count = temp.count(f)
            tp = 0
            fp = 0
            fn = 0
            if original_count == predicted_count:
                tp = original_count
                # tp = 1
            elif original_count < predicted_count:
                tp = original_count
                fp = abs(original_count - predicted_count)
                # fp = 1
            elif original_count > predicted_count:
                tp = predicted_count
                fn = abs(original_count - predicted_count)
                # fn = 1
            result_analysis.append([f, original_count, predicted_count, tp, fp, fn])
    else:
        input_beta_fp = os.path.join(imgs_root, f)
        input_beta = mmcv.imread(input_beta_fp)
        input_beta = cv2.cvtColor(input_beta, cv2.COLOR_BGR2RGB)
        ...
        predicted_boxes = output_beta[0][0]
        predicted_boxes = predicted_boxes[predicted_boxes[:, 4] >= 0.8]
        idx, filtered_boxes = get_filtered_preds(predicted_boxes.tolist())
        filtered_count = len(filtered_boxes)
        predicted_count = len(predicted_boxes)
        for rect in filtered_boxes:
...
df_path = os.path.join(inference_pipeline_path, f'{model_name}-{cfg.MODEL_NAME}-s{cfg.S}Nofilt.csv')
test_results_analysis_df.to_csv(df_path, index=False)
df2_path = os.path.join(inference_pipeline_path, f'{model_name}-{cfg.MODEL_NAME}-s{cfg.S}-Scores_nofilt.csv')
